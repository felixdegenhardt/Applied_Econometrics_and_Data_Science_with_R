...
# Your code here
new_data <- data_long %>% 
  left_join(tutor_info, by = c("tutor")) 
...
1. Create a new variable `high_risk` that equals 1 if `risiko` is greater than 5, and 0 otherwise
...
data_summary <- mydata %>% 
  mutate(high_risk = if_else(risiko > 5, 1, 0)) %>%
  filter(age >= 18 & age <= 30) %>% 
  group_by(female) %>%
  summarise(avg_high_risk = mean(high_risk, na.rm = T))
