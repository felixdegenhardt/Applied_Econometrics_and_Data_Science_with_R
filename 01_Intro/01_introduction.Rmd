...
**Open-source**: R is a powerful tool for data analysis, statistical computing, and programming. One of its major advantages is that it is completely open source, which means that anyone can use and contribute to it without worrying about licensing fees. This open-source nature has fostered a massive, global community of R users who are continuously improving the language. The size of the community also means that if you ever encounter a problem or need a new feature, there’s likely already a package or someone who can help.

**Rich set of packages:** Another reason to choose R is the incredible rich set of packages designed for data science. Whether you’re cleaning data with `dplyr`, visualizing with `ggplot2`, or building dashboards with `shiny`, R has a package for nearly every data science task you can imagine. The package ecosystem makes R especially well-suited for data wrangling and analysis. That is also the reason R is used way beyond the academic environment. 

**Flexibility:** R is also cross-platform, meaning it runs on Windows, Mac, and Linux, and many of the concepts you learn in R (such as object-oriented programming) transfer easily to other languages like Python. This flexibility is a huge advantage, especially for those working in interdisciplinary environments or collaborating across different teams. Moreover, R integrates smoothly with other tools like SQL databases and Python, making it an excellent choice for those working with diverse technologies.

## Why R-Markdown?

**All-in-one:** R-Markdown is an invaluable tool for combining code and narrative into a single, cohesive document. One of its key benefits is the ability to write code that generates immediate output, like graphs, tables, and summaries, which appear directly in the document. Some researchers even write full research papers in R-Markdown, with the great benefit of not having to copy-paste one single element! 

**Reproducibility:** One of the greatest strengths of R-Markdown is its role in ensuring reproducibility. In the world of data science, being able to reproduce an analysis is essential for validation, peer review, and collaboration. R-Markdown allows you to document your entire analysis pipeline in one place, making it clear how the results were generated and ensuring that others can replicate your work exactly.

**Flexibility:** Another significant advantage of R-Markdown is its flexibility when it comes to exporting documents. Whether you need an HTML report for a web page, a PDF for printing, or even a Microsoft Word document for more traditional formats, R-Markdown allows you to easily create multiple types of outputs from the same source file. This flexibility extends to presentations and slides, which can also be generated from the same document.

**Automation:** R-Markdown also simplifies reporting. Once your document is set up, it becomes easy to update. For example, if your dataset changes or new data comes in, you can re-run the analysis, and the entire document, including all outputs, updates automatically.
...
1. The first rule for clean coding is to **always comment your code**. Comments are the best way to explain the *why* behind your decisions. It’s easy to understand *what* the code is doing by reading it, but comments should clarify the intent and purpose behind complex lines or functions.

2. Another key aspect of clean coding is using **meaningful variable names**. Your variable names should convey what they represent without requiring the reader to look up their definitions. For instance, `avg_income` is much more informative than a generic name like `x`.

3. A good principle to follow is the DRY rule, which stands for **“Don’t Repeat Yourself”**. If you find that you are repeating similar lines of code, it is more efficient to refactor those lines into a function (we will cover this of course!) . This not only reduces redundancy but also makes your code easier to maintain and modify if needed.

4. When writing code, aim to **keep each line of code short**. This improves readability, particularly when working with version control systems, or sharing the code with others. Long lines are harder to read and can make it more difficult to spot bugs or errors.

5. Lastly, **consistency is key in coding**. You should always follow a consistent style for indentation, spacing, and naming conventions. For example, choose between using single or double quotes and stick with your choice throughout the project. These small decisions make a big difference in keeping your code readable and professional.
...
- There are many great websites that help you get started and explain how to customize your document.
...
Please create a folder for our class on your laptop. After that, please download our first RMarkdown document from Moodle and first save it in a new folder (e.g. "01_intro") in our class folder. Now you can open it in RStudio and start working in it! 
...
```{r}
mydata <- read.csv(file="../00_data/survey/survey.csv", header=TRUE, sep=",")
```
...
write.table(mydata, file = "../00_data/survey/mydata.txt", sep = "\t")
```

# FYI 

## The `which` function
Sometimes, we want to access a subset of our data, using a specific condition. For example, we would like to calculate the share of all female students that are risk affine. The function `which` hepls us do this efficiently. 

The following gives us a logical true or false response for each individual.

```{r}
# Checks if the values in the risk_affine column of the mydata data frame are 
# equal to 1, and displays the first few results.
head(mydata$risk_affine == 1) 
```
This will now give us the indices of all individuals where the logical response is true.
```{r}
# Identifies the indices of rows in the risk_affine column where the value is 1
# and displays the first few results.
head(which(mydata$risk_affine == 1))
```
We can use these indices to subset this specific group of individuals in our data.How many individuals are under 40 years old?
```{r}
# Calculates and returns the number of rows in the geschlecht column 
# that have the value 1.
length(which(mydata$geschlecht == 1))
```
Now we can calculate the share of risk affine persons within the group female students.

```{r}
# calculates the mean of the risk_affine values for rows where geschlecht is 1, 
# while ignoring missing values.
mean(mydata$risk_affine[which(mydata$geschlecht == 1)], na.rm = TRUE)
```
...
colnames(reg_data) <- c("risk_affine","note","schuhe")
...
lm1 <- lm(note ~ risk_affine, data = reg_data)
...
lm2 <- lm(note ~ risk_affine + schuhe, data = reg_data)
summary(lm2)
```
...
glm1 <- glm(risk_affine ~ note + schuhe, data = reg_data, family = binomial)
summary(glm1)
```
